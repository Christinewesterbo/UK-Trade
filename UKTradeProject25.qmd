...
## Import and prepare gravity data ----
...
mutate(
partner_eu = if_else(iso3_o == "GBR", eu_d, eu_o)
)
...
gravity <- readRDS(here("data", "Gravity_V202211.rds")) %>%
  ...
  filter(
    # beholder kun rader hvor minst én av partene er UK
    iso3_o == "GBR" | iso3_d == "GBR",
    
    # fjern rader der ett av landene ikke eksisterer
    country_exists_o == 1 & country_exists_d == 1,
...
# Skriv ut første rader som sjekk
head(gravity)
...
gravity <- gravity %>%
  mutate(
    partner_eu = if_else(iso3_o == "GBR", eu_d, eu_o)  # EU-status for partnerland
  )

```

```{r}
uk_exports <- gravity %>% filter(iso3_o == "GBR")
uk_imports <- gravity %>% filter(iso3_d == "GBR")
```
...
2.2 Import and prepare CEPII Gravity data
...
2.3 Create export and import datasets
```{r}
uk_exports <- gravity %>% filter(iso3_o == "GBR")
uk_imports <- gravity %>% filter(iso3_d == "GBR")
```
...
# 3.6 Regression analysis: EU vs non-EU trade after 2016
